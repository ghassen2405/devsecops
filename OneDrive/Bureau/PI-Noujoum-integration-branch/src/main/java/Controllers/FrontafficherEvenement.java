...
public class FrontafficherEvenement {
    ...
    private VBox createEventCard(Evenement event) {
        ...
        buttonsContainer.getChildren().addAll(reserveButton, mapButton);
    ...
    public void goToTickets() {
        try {
            // Load the mestickets.fxml file
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/voirTicket.fxml"));
            AnchorPane ticketsPage = loader.load();

            // Create a new Stage for the new scene
            Stage stage = new Stage();
            stage.setScene(new Scene(ticketsPage));
            stage.setTitle("Mes Tickets");
            stage.show();

            // Optionally, close the current stage (if you want to close the current window)
            Stage currentStage = (Stage) btnAll.getScene().getWindow(); // Assuming btnAll is part of this scene
            currentStage.close();
