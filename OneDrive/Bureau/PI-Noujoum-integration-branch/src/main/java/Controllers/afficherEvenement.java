...
public class afficherEvenement {
    ...
    private VBox createEventCard(Evenement event) {
        ...
        buttonsContainer.getChildren().addAll(modifyButton, deleteButton,  mapButton);

        eventCard.getChildren().addAll(eventImageView, eventDetails, buttonsContainer);
        return eventCard;
    }

    public void goToTickets(MouseEvent event) {
        try {
            // Load the "mestickets.fxml" page
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/mestickets.fxml"));
            AnchorPane ticketsPane = loader.load();

            // Create a new scene and stage for the tickets page
            Stage stage = new Stage();
            stage.setScene(new Scene(ticketsPane));
